...
final class TXTParser: BookDocumentParser {
    ...
    private static func chapterHeading(from line: String) -> String? {
        ...
        let chinesePattern = "^(?:第\\s*[0-9０-９一二三四五六七八九十百千万零〇两]+\\s*[章节卷回篇].{0,48}|(?:序章|楔子|前言|后记|尾声|番外).{0,48})$"
        let englishPattern = "^(?i:(?:chapter\\s+(?:[0-9]+|[ivxlcdm]+).{0,48}|prologue|epilogue))$"
        return matches(line, pattern: chinesePattern) || matches(line, pattern: englishPattern) ? line : nil
    }
